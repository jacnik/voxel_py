from settings import (glm, MAX_RAY_DIST, CHUNK_SIZE,
                      CHUNK_AREA, WORLD_H, WORLD_W, WORLD_D, WORLD_AREA)
...
class VoxelHandler:
    ...
    def ray_cast(self):
        ...
        dx = glm.sign(x2 - x1)
        delta_x = min(dx / (x2 - x1),
                      10_000_000.0) if dx != 0 else 10_000_000.0
        max_x = delta_x * (1.0 - glm.fract(x1)
                           ) if dx > 0 else delta_x * glm.fract(x1)

        dy = glm.sign(y2 - y1)
        delta_y = min(dy / (y2 - y1),
                      10_000_000.0) if dy != 0 else 10_000_000.0
        max_y = delta_y * (1.0 - glm.fract(y1)
                           ) if dy > 0 else delta_y * glm.fract(y1)

        dz = glm.sign(z2 - z1)
        delta_z = min(dz / (z2 - z1),
                      10_000_000.0) if dz != 0 else 10_000_000.0
